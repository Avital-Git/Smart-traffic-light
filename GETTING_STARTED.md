...
### 1.2 מסד נתונים SQL Server

#### אפשרות 1: שימוש בבנייה גרפית (SQL Server Management Studio - SSMS)

1. פתח את SQL Server Management Studio (SSMS)
2. התחבר לשרת SQL Server שלך
...
שימי לב: בלי `--with-client` יעלו רק Server + C++ + Auto-Launcher (בלי React UI).
...
המערכת אוטומטית:
- קוראת מ-SQL Server כמה מצלמות בכל צומת (`num_cameras`)
